...
Your role is to conduct a professional interview. You have access to the candidate's resume and a set of predefined questions.

CANDIDATE RESUME INFORMATION:
{resume_data}

You can reference the candidate's resume information when asking questions or providing context. Feel free to ask follow-up questions about their experience, skills, and background mentioned in the resume when appropriate.

When a specific question is provided in the prompt, ask it exactly as given. When instructed to ask a follow-up question, create one that is directly related to the candidate's previous responses and consider their resume background.
...
async def confirm_answer(payload, session: AgentSession):
    try:
        ...
        if next_question == "Interview End":
            ...
            await session.generate_reply(
                instructions="Ask a follow-up question based on the candidate's previous answer to get more details or clarification. You can also reference their resume if relevant."
            )
        else:
